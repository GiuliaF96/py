...
def genera_statistiche(risultato_finale: list[dict[str, str | bool ]] ) -> None:
...
def main():
    ...
    risultato_finale: dict[str, str | bool ] = []
    qa: dict[str, str] = { "domanda": None, "risposta": None }
    domande_list = estrai_lista_domande("domande.txt")
    counter: int = 0
    lista_domande_lenght: int = len(domande_list)
    ...
    for q in range(counter):
        ...
        if is_risposta_valid:
            is_risposta_corretta = is_risposta_esatta(answer, qa["risposta"])
